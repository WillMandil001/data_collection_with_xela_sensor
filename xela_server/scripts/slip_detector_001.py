	...
	def sub_cb(self, tactile_data):
		...
		if self.time_flag==0:
			...
			if len(xela_data) > 20:
				...
				#xela_seq.reshape((1, 20, 96))
				xela_seq = scale(xela_seq, scaler_tactile)
				print(xela_seq)
				...
				robot_seq = np.asarray(robot_traj[current_time_index-20:current_time_index])
				action_seq = np.asarray(robot_traj[current_time_index:current_time_index+20])
			...
			self.delta_t = time.time() - self.loop_start_time
...
if __name__ == "__main__":
	...
	xela_data = np.reshape(xela_data, (xela_data.shape[0], 96))
	T = pd.DataFrame(xela_data)
	T.to_csv('/home/kiyanoush/Desktop/xela_data.csv', header=None, index=False)
